// Free, fully local natural-language parsing for the reminder mic button —
// no AI API call, no cost. Understands English (+ romanized Hindi/Gujarati)
// and native Gujarati/Hindi script for the most common reminder phrasing.
// Good-enough guess; the user reviews/edits in the form before saving, so it
...
export function parseReminderText(rawText, now = new Date()) {
  ...
  // If neither a date word nor an "in N ..." phrase was found and the
  // resolved time has already passed today, assume they mean tomorrow.
  if (!dateSet && result.getTime() <= now.getTime()) {
    result.setDate(result.getDate() + 1);
  }

  // --- Title: whatever's left after stripping recognized phrases ---
  let title = text
    .replace(/\s+/g, ' ')
    .trim()
  ...
  return { title, datetime: result.toISOString(), recurrence, priority, category: '', leadMinutes: 0 };
}
